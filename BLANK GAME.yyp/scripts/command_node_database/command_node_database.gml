function command_node_database(){

	command_node_DB[0] = {
		name : "Down Left",
		description : "Moves the target right",
		command_script : move_prep,
		command_vars : 0,
		sprite : sNodeArrow2,
		priority : STEP_PRIOTITY.MOVE,
		can_appear_in_shops : true,
		rarity : RARITY.COMMON
	}

	command_node_DB[1] = {
		name : "Down Right",
		description : "Moves the target up",
		command_script : move_prep,
		command_vars : 1, //Not required, but if put it will run the command with the arg(s)
		sprite : sNodeArrow3,
		priority : STEP_PRIOTITY.MOVE,
		can_appear_in_shops : true,
		rarity : RARITY.COMMON
	}

	command_node_DB[2] = {
		name : "Up-Right",
		description : "Moves the target left",
		command_script : move_prep,
		command_vars : 2, //Not required, but if put it will run the command with the arg(s)
		sprite : sNodeArrow0,
		priority : STEP_PRIOTITY.MOVE,
		can_appear_in_shops : true,
		rarity : RARITY.COMMON
	}


	command_node_DB[3] = {
		name : "Up-left",
		description : "Moves the target down",
		command_script : move_prep,
		command_vars : 3, //Not required, but if put it will run the command with the arg(s)
		sprite : sNodeArrow1,
		priority : STEP_PRIOTITY.MOVE,
		can_appear_in_shops : true,
		rarity : RARITY.COMMON
	}
	
	command_node_DB[4] = {
		name : "Heal",
		description : "Heal 1 health",
		command_script : buff_entity,
		command_vars : [BUFFS.HEAL,1], //Not required, but if put it will run the command with the arg(s)
		sprite : sHitDebris,
		priority : STEP_PRIOTITY.POSTSTEP,
		can_appear_in_shops : true,
		rarity : RARITY.UNCOMMON
	}

	command_node_DB[5] = {
		name : "Move To Player",
		description : "Move towards the player, this is useless to you",
		command_script : move_to_player,
		cost  : 0,
		sell_price : 3,
		sprite : sHitDebris,
		priority : STEP_PRIOTITY.MOVE,
		can_appear_in_shops : false,
		rarity : RARITY.COMMON
	}
	
	command_node_DB[6] = {
		name : "Move To Enemy",
		description : "Move towards the closest enemy",
		command_script : move_to_enemy,
		cost  : 0,
		sell_price : 3,
		sprite : sHitDebris,
		priority : STEP_PRIOTITY.MOVE,
		can_appear_in_shops : true,
		rarity : RARITY.UNCOMMON
	}
	
	
	command_node_DB[7] = {
		name : "Single Attack",
		description : "Attack the tile infront of you",
		command_script : attack_dir,
		sprite : sHitDebris,
		priority : STEP_PRIOTITY.ATTACK,
		can_appear_in_shops : true,
		rarity : RARITY.COMMON
	}
	




}